...
11. 각 사용자로 로그인해 **내 공간**이 서로 다른 빈 폴더인지 확인하고, 한 사용자의 하위 폴더를 다른 사용자에게 공유해 **공유 폴더**에만 표시되는지 확인한다.
...
- 잘못 공유한 폴더: 소유자가 폴더 행의 **공유**에서 대상 사용자의 선택을 해제한다. 공유를 해제해도 파일은 소유자의 개인 공간에 그대로 남는다.
...
- 현재 공유 권한은 `editor` 하나이며, 공유자는 해당 폴더와 하위 폴더를 관리할 수 있다. 읽기 전용 권한과 파일 단위 공유는 후속 범위다.
